...
//	
	...
	public void getComments() throws IOException, WeiboException
	{
		...
		String retweetIdsFile = "DATA/newdownload/2/uid-nickname-tid-rtid-rttext-date-source-ctuid-deletenone.txt";  //已获取微博id和tweetcount的文件路径
		FileUtil fu = new FileUtil();
		String[] tokens = {                            //七个应用的七个token循环调用
				"2.00DdJ8lBQ_jEAE490b754e3e0c5a1g",
				"2.00DdJ8lBTBYz6E9f73f5c4baNsN_cC",
				"2.00DdJ8lB0JbR_Sadcd3a7f8106UIIq",
				"2.00DdJ8lBSiwMBEdeca8baebc_KgpAC",
				"2.00DdJ8lB0xzLPBc343457fbe0ne9h2",
				"2.00DdJ8lBpRRC5Ef34bafd703LpkGfE",
				"2.00DdJ8lB0FVbraf098e516d802RJ5f"
				}; 
		String path = "DATA/newdownload/2/uid-nickname-tid-rtid-rttext-date-source-ctuid";  //保存获取到的数据的文件前缀
		String format = ".txt";
		BufferedWriter bw;
		String[] words;
		...
		for(int i = 0; i < tokens.length; i++)  //循环调用token
		{
			...
			try{			
				...
				{
					words = l.split(",,");
					getComment(words[0],words[1],words[2],Integer.parseInt(words[7]),bw);
				}
			}catch(Exception e)
			{
				e.getStackTrace();
	...
	{ 
	       ...
	       timer.schedule(task, 1* 1000, 3750  * 1000);
	} 
