...
public class FileUtil {
	 ...
	 * @throws IOException 
	 */
	public void mergeFiles(String path,String fromFileName,String toFileName,int start,int end,String format) throws IOException
	{
		String toFilePath = path+toFileName+format;
		BufferedWriter bw = writeFile(toFilePath);
		for(int i = start; i <= end; i++)
		{
			System.out.println("开始合并第"+i+"个文件....");
	...
	public List<String> ReadAllFiles(String filePath)
	{
		...
		{			
				...
				{
					file = new File(filePath+file1[i]);
	 ...
	 * @throws IOException 
	 */
	public void deleteDuplicateID(String fromPath,String toPath) throws IOException{
		int i = 0,lineNo = 1;
		...
		while((line = br.readLine()) != null){
			System.out.println("正在进行第"+lineNo+"行....");
				if(!ids.contains(line.split(",")[1])){
	 ...
	 * @throws IOException
	 */
	public void deleteDuplicateLine(String fromPath,String toPath) throws IOException{
		int i = 0,lineNo = 1;
		List<String> ids = new ArrayList<String>();
		String line;
		String[] words;
		...
		while((line = br.readLine()) != null){
			System.out.println("正在进行第"+lineNo+"行....");
			words = line.split(",");
				if(!ids.contains(words[0]+","+words[1])){  //关注关系
					ids.add(words[0]+","+words[1]);
					bw.write(line);
					bw.newLine();
			}
			lineNo++;
