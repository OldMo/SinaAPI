...
import cn.edu.ccnu.pack.ComputeLinks;
...
public class PageRank {
	...
	public PageRank(double[][] matrix){
		pageRank = new double[matrix.length];
		this.matrix = matrix; 
		System.out.println("weightmatrix:"+ matrix.length);
			...
			for(int k = 0; k < tUsers.length; k++){
				if(j == tUsers[k]){                                      						//如果j为要评估的人的序号，那么就对j在tUsers中对应的列求PR总和
					for(int l = 0;l < matrix.length; l++){
							sum += pageRank[l]*matrix[l][k]/outLinks[l]; 		//计算各个PR总和
					}
				}
			}
...
//		String commentFile = "DATA/105人/commentmatrix.txt";
		...
		List<Edge> edges = mm.mergeEdges(followFile, retweetFile, commentFile, weight);      //获取三个网络结合后的所有的边
		...
		double[][] matrix = mm.initialMatrix(allNodes, toUsers, tUsers, edges);										//构建的关系矩阵													//通过关系矩阵得到的权重矩阵
		
		PageRank wpr = new PageRank(matrix);
