...
//		 }
	 ...
	 * @throws IOException
	 */
	public List mergeEdges(String followFile,String retweetFile,String commentFile,int weight) throws IOException{
		int totleSizes = 0;
		...
		System.out.println("评论网的边数:"+redges.size());
		...
		for(Edge fedge:fedges){
				if(!redges.contains(fedge)){                
					 ...
					 int fside = fedges.get(findex).getSides();
					 int rside = redges.get(rindex).getSides();
					 Edge ed = redges.get(rindex);
					 ed.setSides(rside+weight*fside);                   		//根据权重重新设置边的值
		...
		for(Edge cedge:cedges){
			if(!redges.contains(cedge)){                
				 ...
				 int cside = cedges.get(cindex).getSides();
				 int rside = redges.get(rindex).getSides();
				 Edge ed = redges.get(rindex);
				 ed.setSides(rside+cside);                  							 //评论的权重设置跟转发一致
				 redges.set(rindex, ed);
				 repite++;
			 }
		}
	 ...
	 * @param fromGraph .txt文件
	 */
	public List getToUsers(List<Node> allNodes,String fromGraph) throws IOException{
